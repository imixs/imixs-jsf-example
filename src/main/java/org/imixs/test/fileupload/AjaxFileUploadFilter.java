...
public class AjaxFileUploadFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		fileDataList = (List<MultiFileData>) httpRequest.getSession()
		...
		if (isGetFileUploadRequest(httpRequest)) {
			...
			MultiFileData fileData = getFile(filename);
	...
	private void writeFileContent(ServletResponse response,
			MultiFileData fileData) throws IOException {
	 ...
	 *            - filename to be removed
	 */
	private MultiFileData getFile(String file) {
		MultiFileData result = null;
		if (file == null)
			return null;
		for (int i = 0; i < fileDataList.size(); i++) {
			MultiFileData fileData = fileDataList.get(i);
